<script type='text/javascript'>
    function createAttribute(obj) {
        var select = $('#Offer_offerAttributes'),
            outer, label;
        ...
        select.parent().append('<label for="Offer_offerAttributes_' + obj.id + '">' + obj.name + '</label>');
        if(obj.type_id == <?php echo Attribute::TYPE_LIST;?>) {
            outer = $('<select>')
                .addClass('span7 popover-help')
                .attr({
                    id: 'Offer_offerAttributes_' + obj.id,
                    name: 'Offer[offerAttributes][' + obj.id + ']',
                    type: 'text'
                });
...
//            obj.value_list = obj.value_list.split('\n');
            ...
            outer = $('<span>').attr('id', 'Offer_offerAttributes_' + obj.id);
            select.parent().append(outer);
//            obj.value_list = obj.value_list.split('\n');
            $.each(obj.value_list, function(key, value) {
...
//                        id: 'Offer_offerAttributes_' + obj.id,
    ...
    $(document).ready(function(){
        ...
        $('#Offer_offerAttributes').on('change', function(event){
...
$form = $this->beginWidget('bootstrap.widgets.TbActiveForm', array(
    ...
    <div class="row">
        ...
        <?php
        $options = array();
        foreach(Attribute::model()->onlyOffers()->findAll() as $val) {
        ...
        $attrs = CHtml::listData(Attribute::model()->onlyOffers()->findAll(), 'id', 'name');
